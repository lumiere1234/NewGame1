...
Shader "Custom/Equip alpha" {
	Properties {
        _MainTex ("Base (RGB)", 2D) = "white" {}

		//_LightmapTex ("光照图 (RGB)", 2D) = "white" {}
 		//_MaxLight ("lightmap mul", Range(0,1)) = 1
		...
		_AlphaRef ("AlphaTest Ref", Range(0,1)) = 0
	...
	{ 
		...
		float2 luv = float2( Input.uvBase.x * 0.5, Input.uvBase.y);
		float2 ruv = float2( Input.uvBase.x * 0.5 + 0.5, Input.uvBase.y);
		...
		clip ( colBaseAlphaTex.a - _AlphaRef);
...
//		#endif
		...
		float fNDotL = 1.0f; // dot(Normal, LightDir);
		...
		//colAmbient = UNITY_LIGHTMODEL_AMBIENT.rgb*2;
		...
		//#ifdef SPECULAR_MAP_ENABLE
			fSpecPower = _Power * colSpecularTex.r;
		...
		//#endif
			...
			fSpecPower = _Power;
			vSpecColor = _SpecularMultiple * _colMtlSpecular.rgb;
		#endif
	...
	ENDCG
	SubShader {
		Tags {"Queue"="Transparent" "LightMode"="ForwardBase"  "IgnoreProjector"="True" "RenderType"="TransparentCutout"}
		LOD 200

	    Pass {
			AlphaTest Greater [_AlphaRef]
