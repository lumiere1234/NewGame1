...
// | R | G |
// ---------
Shader "Custom/Seven Equip..Host" {
	Properties
	{
	...
	CGINCLUDE
		...
		float4 frag (v2f i) : COLOR
		{
			...
			fixed3 texRamp = tex2D(_RampMap, rampUV).rgb ;
			...
			albedo = maskColor.b* 2.0 * tex2D (_LightTex, i.capCoord).xyz +  col.xyz * _fDiffusePower * _colDiffuse.rgb + noise;
 			...
 			float3 result = (texRamp + _AmbientColor) * albedo * _SevenBloomEquipCorrect;
