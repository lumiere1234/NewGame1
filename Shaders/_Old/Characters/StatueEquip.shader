...
Shader "Custom/Statue Equip.." {
	Properties
	{
		...
		_StatueMulti("Statue Multiply", float) = 1
		//MatFs
	...
	CGINCLUDE
		...
		float4 frag (v2f i) : COLOR
		{
			...
			fixed3 mask = tex2D(_MaskMap, i.uv);
		 	...
		 	fixed3 stoneTexture = tex2D(_StatueTex, i.uv);
		 	col = stoneTexture * lerp( 1, Luminance(col) * 1.25 , _TextureAmount ) *_StatueMulti ;
		 	...
		 	fixed3 matcap = fixed3(0,0,0);
				...
				matcap += tex2D(_EnvMap,i.capCoord_lowerRt ) * mask.g;	
				matcap += tex2D(_EnvMap,i.capCoord_upperLf ) * mask.b;	
				matcap += (tex2D(_EnvMap,i.capCoord_upperRt ) * maskB.g).ggg;	// remove color, this channel is used for hair
		 	#endif
		 	matcap *= 2;
		 	...
		 	//	fixed base = 1-t.x-t.y - i.color.r;
		 		// col.rgb *= fixed3(base,base,base) + 2*( t.x * _Tint.a * _Tint.rgb + t.y *_Tint2.a * _Tint2.rgb + i.color.r * _Tint3.rgb *_Tint3.a);
		 	//	col.rgb *= _Tint.rgb;
		 	#endif
