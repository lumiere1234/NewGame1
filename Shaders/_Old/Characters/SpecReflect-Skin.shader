...
Shader "Custom/Skin" {
	...
	{ 
		...
		Normal = glb_SamplerNormalMap(_MainTex, ruv, _smoothNess);
		...
		float4 colSpecularTex = float4(1,1,1,0);//R:power  G: gloss B: Reflect
		...
		colDiffuse = colSunDiffuse * saturate(fNDotL);

		//Ambient
		colAmbient = (_colAmbient.rgb * fA1) + (_colAmbient2.rgb * fA2);
