...
Shader "Chp/Scroll/Additive Mask 2 Scroll " {
Properties {
...
SubShader {
	...
	pass{
		...
		fixed4 frag(vertexOutput i) : COLOR
		{
			...
			fixed3 mainTex = tex2D(_MainTex,i.uv_MainTex)  * _Color01;
			fixed3 blendTex = tex2D(_Blend_Texture,i.uv_Blend_Texture) * _Color02;
			fixed3 blendTex1 = tex2D(_Blend_Texture01,i.uv_Blend_Texture01)* _Color03;
			
			color = fixed4( (mainTex.rgb + blendTex.rgb  + blendTex1.rgb ) * length(_Lighten*mainTex) , 1) * i.color ;
			return color; 
		}
		ENDCG
	}
