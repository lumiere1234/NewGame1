...
Shader "Chp/Disslove/Disslove xRay"
{
	Properties
	{
    ...
    fixed4 frag (v2f i) : COLOR0 
	{
		...
		fixed4 output = fixed4(color.rgb , alpha * color.a);// _Transparent * Luminance(color.rgb));
		return output;

	}

	fixed4 fragZ (v2f i) : COLOR0
	{
		float edge = pow(i.frenel, _Range);
		fixed4 color = tex2D(_MainTex,i.uv) ;
		float l = saturate( Luminance(color.rgb) + _Mix.w);
		half alpha =  (edge * _Mix.x + l * _Mix.y ) * _Mix.z * color.a;
		...
		fixed4 output = fixed4(color.rgb , alpha);// _Transparent * Luminance(color.rgb));
		return output;
	}
