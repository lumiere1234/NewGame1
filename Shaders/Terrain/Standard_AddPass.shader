...
Shader "Hidden/Terrain/Standard_AddPass"
{
	Properties
	{
	...
	{
		Tags { "Queue"="Geometry-99" "LightMode"="ForwardBase"  "IgnoreProjector"="False" "RenderType"="Opaque" }
		Blend One One
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				//虽然正常画，但是依然会导致有些地方的weight小于1(splat_control的rgba相加小于1,会导致第二个SplateTexture与前一个融合时出现黑边),所以把剩下的部分也给个颜色值
				col += col * (1 - weight);
				...
				#if ZL_LIGHTMAP_PAINT_ON
					//i.uvBase.z = clamp(i.uvBase.z,0.01,1);
					//i.uvBase.w = clamp(i.uvBase.w,0.01,1);
				...
				float spcularControlMap = Luminance(col.rgb);
					...
					//虽然正常画，但是依然会导致有些地方的weight小于1(splat_control的rgba相加小于1,会导致第二个SplateTexture与前一个融合时出现黑边),所以把剩下的部分也给个颜色值
					normalColor += normalColor * (1 - weight);

					//normalColor = normalize(normalColor);//可以去掉第二个SplateTexture与前一个融合时出现的法线变平，产生黑边的问题，但是会导致法线效果变浅
					//normalColor = lerp(half4(0,0,1,0),normalize(normalColor),weight);
				...
				//Fog
				half3 addPassFogColor = half3(1,1,1) * splat_control.r * splat_control.g * splat_control.b * splat_control.a;
				UNITY_APPLY_FOG_COLOR(i.fogCoord,col,addPassFogColor);
