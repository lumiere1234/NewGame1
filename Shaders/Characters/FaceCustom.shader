...
Shader "* Character/Skin And Eyeball Custom" {
	Properties {
		...
		_FxColor( "fxColor", Color) = (0, 0.5, 1, 1)
		_FxTexture( "Fx Texture", 2D) = "black" {}
		_FxTextureB( "Fx TextureB", 2D) = "black" {}
		_FxUV("Fx UV", Vector) = (0.5,0.5, 1, 1)
		_FxWeights( "Fx Weight", Vector) = (1,1,1,1)

		_FxThreshold( "FxTreshold", float) = 0


		_Parallax ("_Parallax Height", Range (-2,2)) = 1
   }
	...
	float4 _FxColor;
	sampler2D _FxTexture;
	sampler2D _FxTextureB;
	float4 _FxUV;
	float4 _FxWeights;
	float _FxThreshold;
	float _Parallax;
    ...
    struct v2f {
		...
		float3 eyeLightOffset	: TEXCOORD4;
   ...
   struct v2f_Fx{
    ...
    v2f vert (appdata_full v) {
		...
		o.eyeLightOffset = mul (rotation, ObjSpaceViewDir(v.vertex));
 ...
 fixed4 frag (v2f Input) : COLOR
	...
	//return Input.eyeLightOffset.rgbr;
		...
		*/
		...
		half2 offset = ParallaxOffset ( 0 , _Parallax, Input.eyeLightOffset);
		fixed4 colReflectTex = tex2D(_ReflectionTex, Input.uvBase.zw + _EyeReflectionUVOffset.xy * offset + _EyeReflectionUVOffset.zw);
		float eye_power = pow(colReflectTex.r, _EyeLightGlossiness) * _EyeLightAmont;
		#if ZL_DEBUG_MODE_ON
    ...
    v2f_Fx vertFx(appdata_full v){
        v2f_Fx o;
        o.pos = UnityObjectToClipPos( v.vertex );
        o.uvBase = v.texcoord;
        half t =    saturate( _Time.y % 1 ) ;
        t = t * t  * ( 3- 2 * t);
        half scale = lerp( 0.03, 0.8, t);

        o.uv2.xy = (v.texcoord.xy - _FxUV.xy) / scale +_FxUV.xy;

        return o;
    }

    fixed4 fragFx (v2f_Fx Input) : COLOR
    {
    	half4 fxMask = tex2D(_FxTexture, Input.uvBase);
    	half4 fxAdd = tex2D(_FxTextureB, Input.uv2);
    	
    	half f = dot( fxMask.xyz, _FxWeights.xyz ); 
    	half add = step( 0.5, f) * fxAdd  * 5;
    	
    	return _FxColor * (f + add);
    }
